...
## Si este repositorio ya tiene código escrito antes de usar este sistema de guías

`01_guia_analisis_proyecto.md` tiene una Sección 0 que se ejecuta una sola vez por repositorio (no en cada RF): compara los nombres de tabla, columna y función que el código de `/backend` y `/frontend` ya usa contra el MER y el diagrama de clases, para detectar si hay divergencias antes de construir el primer RF nuevo encima. Esa sección queda registrada en `docs/proyecto/diagnostico_divergencia.md` una vez ejecutada, y se salta sola en sesiones futuras si el código no cambió desde entonces — no hay que pedirla explícitamente, la Guía 01 la ejecuta por su cuenta como primer paso.
...
/docs/guias/
  ...
  07_guia_sistema_color.md          ← Sistema de color (Material 3 / HCT): primario, secundario, derivados, negro/blanco.

**Regla de idioma aplicada en todas las guías anteriores:** variables, métodos, clases, tablas y variables CSS que representan conceptos del dominio de este proyecto se escriben en español (igual que el MER y el diagrama de clases). Los identificadores técnicos fijos de cada framework o lenguaje se mantienen como esa tecnología los define.
```
...
/docs/proyecto/
  ...
  Wireframes_-_Marketplace_Mayorista.html ← Wireframe mid-fi, fuente de la estructura visual por pantalla/RF.
```
